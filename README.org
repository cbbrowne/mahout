...
#+END_EXAMPLE
     (where "1.1" is replaced with the apropos version label).  The
     slonik script references files in ~.mahout-temp~, as well as the
     DDL scripts in the Mahout installation so should be run $PWD
     being set to the ~mahout~ directory containing ~mahout.conf~ and
     ~mahout.control~.
...
** Contents of mahout.conf
  ...
  - URIs to indicate how to access databases of interest
    ...
    - MAHOUTSEARCHPATH :: Indicates a value set as the default
         ...
         - If run via ~mahout upgrade~, the user's default search path
           will be attached in each step;
         - If running via ~mahout slonik~, the ~slony~ user's default
           search path would be attached to the /first/ DDL/DML step;
           subsequent DDL/DML steps will inherit whatever
           ~search_path~ is in effect at the end of the previous step.
